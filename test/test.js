...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dcartesianPower.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the Cartesian power (row-major, k=2)', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
...
tape( 'the function computes the Cartesian power (row-major, k=3)', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
	...
	x = vector( new Float64Array( [ 1.0, 2.0 ] ), 2, 1, 0 );
	out = matrix( new Float64Array( 24 ), 8, 3, 3, 1, 0 );
...
tape( 'the function computes the Cartesian power (row-major, N=3, k=2)', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
	...
	out = matrix( new Float64Array( 18 ), 9, 2, 2, 1, 0 );
...
tape( 'the function computes the Cartesian power (row-major, N=1, k=1)', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
	...
	out = matrix( new Float64Array( 1 ), 1, 1, 1, 1, 0 );
...
tape( 'the function computes the Cartesian power (column-major)', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
...
tape( 'if provided `N` or `k` less than or equal to `0`, the function returns the output ndarray unchanged', function test( t ) {
	var expected;
	var out;
	var x;
	var k;
...
tape( 'the function supports ndarrays having non-zero offsets', function test( t ) {
	...
	expected = new Float64Array( [ 0.0, 0.0, 1.0, 1.0, 1.0, 2.0, 2.0, 1.0, 2.0, 2.0 ] );
	t.strictEqual( v, out, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
	t.end();
});
